...
A list of `n` statements, where statement `i` says: **"exactly `i+1` of the statements in this list are false"**.
...
You need to decide, for each statement, whether it is true or false, such that its truth value matches reality.

Represent an assignment as `answers : Vector Bool n`, where `answers[i] = true` means statement `i` is **true**. It is consistent when `answers[i] ↔ answers.count false = i + 1`.

Your task: prove that for every `n > 1`, a consistent assignment **exists and is unique**.
